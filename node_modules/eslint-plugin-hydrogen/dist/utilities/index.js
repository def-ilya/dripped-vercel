...
exports.createRule = experimental_utils_1.ESLintUtils.RuleCreator((name) => {
    const ruleName = path_1.parse(name).name;
...
function getRepoFromPackageJson(pkg) {
    const repoPathParts = path_1.parse(pkg.repository.url);
    return path_1.join(repoPathParts.dir, repoPathParts.name, pkg.repository.directory);
}
const deepCopy = (obj) => {
    if (typeof obj === 'object') {
        const copyArray = (arr) => arr.map((val) => exports.deepCopy(val));
        ...
        for (const key in obj) {
            ...
            if (val instanceof Array) {
                ...
                newObj[key] = exports.deepCopy(val);
            }
            else {
                newObj[key] = val;
            }
...
function merge(from, to) {
    const mergedInto = exports.deepCopy(to);
